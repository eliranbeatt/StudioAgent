...
### 2.4 Convex `schema.ts` patch (clean, copy-paste friendly)
```ts
...
export const WORK_TYPE_LABELS_HE: Record<string, string> = {
  planning_production: "תכנון/הפקה",
  design_art_direction: "ארט/עיצוב",
  procurement_pickups: "קניות/איסופים",
  vendor_management: "ספקים/הזמנות",
...
export default defineSchema({
  tasks: defineTable({
    ...
    // ...keep all your existing task fields here unchanged
  })
...
**UI rule:** never show the enum key (`printing_graphics`). Render `workTypeLabelHe` if present, otherwise `WORK_TYPE_LABELS_HE[workType]`.
