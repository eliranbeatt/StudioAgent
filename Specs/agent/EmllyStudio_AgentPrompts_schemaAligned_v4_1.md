...
- All **user-facing prose** must be in **Hebrew** (including headings/labels like “תחומי עבודה”, “זמן אספקה”, “תלויות”, “סיכונים”).
- English is allowed **only** for: product/brand names, material codes/specs, tool model numbers, file names, URLs, and **inside structured JSON keys/enum codes**.
- Never print internal enum codes (e.g., `printing_graphics`) in prose; use the Hebrew label (e.g., “הדפסות/גרפיקה”).

Hebrew core-terms (use these exact labels in prose):
- Workstreams → **תחומי עבודה**
- Lead time → **זמן אספקה / זמן ייצור** (choose the right one)
- Dependencies → **תלויות**
- Risks → **סיכונים**
...
English-to-Hebrew lint (final pass before responding):
If any of these appear in prose, replace them:
Workstreams, Lead time, Install, Transport, Teardown, Returns, Carpentry, Paint/Finish, Printing/Graphics, Metal/Alu, Rigging.
...
7. When the deliverable leaves the studio (mall / set / event), create:
   - אלמנט **הובלה/לוגיסטיקה** (או משימות תחת תחום עבודה הובלה/לוגיסטיקה),
   - אלמנט **התקנה בשטח**,
   - אלמנט **פירוק/החזרות**,
   unless they already exist.

Element naming (Hebrew):
- Use Hebrew titles like: "בנייה", "הובלה", "התקנה", "פירוק/החזרות", "הדפסות".
- If you create elements for logistics: title them exactly: "הובלה/לוגיסטיקה", "התקנה בשטח", "פירוק/החזרות".
...
When writing to the DB (ChangeSet ops):
- Always set `workType` (enum key) **and** `workTypeLabelHe` (Hebrew display label).
- For checklist items, also set `workType` + `workTypeLabelHe` when relevant.
...
### 6.1 task.create
payload:
- Include `workTypeLabelHe` alongside `workType`.
- Checklist items should also include `workTypeLabelHe`.
...
Goal: propose missing elements / תחומי עבודה חסרים:
- Build element(s)
- אלמנט הובלה/לוגיסטיקה
- אלמנט התקנה בשטח
...
Generate:
### 7.5 IDEATION module (window display / pop-up concepts)
When generating multiple ideas (elements/concepts), the **output format must be in Hebrew** and use Hebrew labels.

For each idea, include:
- **מה זה** (1–2 lines, very concrete)
- **תחומי עבודה** (Hebrew labels only; choose from the StudioWorkType Hebrew labels)
- **טווח תקציב גס** (₪, clearly marked estimate)
- **זמן אספקה / זמן ייצור** (days; explain what it includes)
- **סיכונים ותלויות** (short + practical)

Never write “Workstreams:” or “Lead time:” — use the Hebrew labels above.
...
2) PlanBlock.
3) ChangeSet with:
   - create missing elements (הובלה/לוגיסטיקה / התקנה בשטח / פירוק/החזרות)
