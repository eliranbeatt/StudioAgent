...
export default function AccountingPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const breakdown = useQuery(api.financials.getDraftCostBreakdown, { projectId });

  if (!summary || !breakdown) return <div className="p-8">Loading accounting data...</div>;
        ...
        {stats.map((stat) => (
          <div key={stat.label} className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-all">
            ...
            <div className="text-3xl font-bold text-gray-900">
              {Number(stat.value).toLocaleString()} <span className="text-lg text-gray-400 font-normal">NIS</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white border border-gray-100 rounded-xl overflow-hidden shadow-sm">
          <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
            <span className="font-bold text-gray-900 text-sm">Cost Breakdown (Draft Forecast)</span>
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center text-sm group cursor-pointer hover:bg-gray-50 p-2 rounded transition-colors">
                ...
                <span className="font-mono text-gray-900">
                  {breakdown.totals.directCost === 0 ? "--" : breakdown.elementCosts.materials.toLocaleString()} NIS
                </span>
              </div>
              <div className="flex justify-between items-center text-sm group cursor-pointer hover:bg-gray-50 p-2 rounded transition-colors">
                <span className="text-gray-600 font-medium">Labor & Production</span>
                <span className="font-mono text-gray-900">
                  {breakdown.totals.directCost === 0 ? "--" : breakdown.elementCosts.labor.toLocaleString()} NIS
                </span>
              </div>
              <div className="flex justify-between items-center text-sm group cursor-pointer hover:bg-gray-50 p-2 rounded transition-colors">
                <span className="text-gray-600 font-medium">Subcontractors</span>
                <span className="font-mono text-gray-900">
                  {breakdown.totals.directCost === 0 ? "--" : breakdown.elementCosts.subcontract.toLocaleString()} NIS
                </span>
              </div>
              <div className="h-px bg-gray-100 my-2" />
              <div className="flex justify-between items-center font-bold text-gray-900 p-2">
                <span>Total Direct Cost</span>
                <span className="font-mono text-lg">{breakdown.totals.directCost.toLocaleString()} NIS</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="p-6">
            <div className="space-y-4">
              <div className="flex justify-between items-center text-sm p-2">
                ...
                <span className="font-mono text-gray-900">{summary.effectiveBudget.directCost.toLocaleString()} NIS</span>
              </div>
              <div className="flex justify-between items-center text-sm p-2">
                <span className="text-gray-600 font-medium">Overhead & Risk</span>
                <span className="font-mono text-gray-900">{(summary.effectiveBudget.sellPrice - summary.effectiveBudget.directCost).toLocaleString()} NIS</span>
              </div>
              <div className="h-px bg-gray-100 my-2" />
              <div className="flex justify-between items-center font-bold p-2 bg-gray-50 rounded-lg border border-gray-100">
                ...
                <span className="font-mono text-lg text-gray-900">{summary.effectiveBudget.sellPrice.toLocaleString()} NIS</span>
              </div>
            </div>
          </div>
        </div>
