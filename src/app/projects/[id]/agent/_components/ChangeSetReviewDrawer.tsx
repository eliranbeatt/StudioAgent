              ...
              return (
                <div 
                  ...
                  className={`bg-white border rounded-lg shadow-sm transition-all overflow-hidden ${
                    isEditing ? "ring-2 ring-blue-500 border-transparent" : "border-slate-200"
                  }`}
                >
                  {/* Row Header */}
                  <div 
                    ...
                    <div className="flex-1 flex flex-col gap-0.5">
                      ...
                      {isExpanded && !isEditing && (
                         <div className="text-[10px] text-slate-400 pl-1 font-mono">
                            REF: {op.payload?.tempId ?? op.payload?.taskId ?? op.payload?.elementId ?? "New"}
                         </div>
                      )}
                    </div>
...
function renderFieldValue(key: string, value: any) {
    ...
    return <span className="text-sm text-slate-700 font-medium break-words leading-relaxed">{String(value)}</span>;
}
