...
export function SuggestionBlock({ block, onSubmit }: { block: any, onSubmit?: (text: string) => void }) {
  const [note, setNote] = useState("");

  const handleSelect = (item: any) => {
      const text = item.payload?.action 
        ? `Run skill: ${item.payload.action}` 
        : `Selected: ${item.labelHe || item.label || item.label_he}`;
