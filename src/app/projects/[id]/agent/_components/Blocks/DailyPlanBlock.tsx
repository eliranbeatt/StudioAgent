...
export function DailyPlanBlock({ block }: { block: any }) {
  ...
  return (
    <div className="rounded-xl border border-cyan-200 bg-white p-4 shadow-sm space-y-3" dir="auto">
      <div className="text-sm font-semibold text-gray-900">Daily Plan</div>
      {date && <div className="text-xs text-cyan-800">{date}</div>}
      {priorities.length > 0 && (
        <div className="text-xs text-cyan-900 border-t border-cyan-100 pt-2">
      ...
      {schedule.length > 0 && (
        <div className="text-xs text-cyan-800 border-t border-cyan-100 pt-2">
