...
export const getConfig = query({
    ...
    handler: async (ctx, args) => {
        const project = await ctx.db.get(args.projectId);
        // @ts-ignore
        return project?.tasksConfiguration?.trelloConfig ?? null;
    }
});
...
export const createBoard = action({
    ...
    handler: async (ctx, args) => {
        ...
        const res = await fetch(`https://api.trello.com/1/boards/?name=${encodeURIComponent(args.name)}&defaultLists=true&key=${key}&token=${token}`);
        if (!res.ok) {
            const err = await res.text();
...
export const sync = action({
  ...
  handler: async (ctx, args) => {
      ...
      try {
          ...
          const cardsByName = new Map(cards.map((c: any) => [c.name, c]));
