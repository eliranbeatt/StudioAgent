...
import { isEnabled, normalizeFlags, DEFAULT_FLAGS } from "../featureFlags";
...
export const sendMessageAndRun = action({
  ...
  handler: async (ctx, args) => {
    // 1. Save User Message
    await ctx.runMutation(internal.skills.runner.sendUserMessage, {
    ...
    if (conversation && conversation.title === "New Session") {
      const messages = await ctx.runQuery(api.skills.actions.listAgentMessagesAction, { conversationId: args.conversationId });
...
export const listAgentMessagesAction = action({
  args: { conversationId: v.id("agentConversations") },
  handler: async (ctx, args) => {
    return await ctx.runQuery(api.skills.runner.listAgentMessages, { conversationId: args.conversationId });
  }
});
